...
public class CartController {
    ...
    @ResponseBody
    public com.toky.shop.pojo.Msg getCart(HttpSession session) {
        User user = (User) session.getAttribute("user");
        if(user == null) {
        ...
        return com.toky.shop.pojo.Msg.success("查询成功").add("shopcart",goodsAndImage);
    }

    @RequestMapping(value = "/deleteCart/{goodsid}", method = RequestMethod.DELETE)
    @ResponseBody
    public com.toky.shop.pojo.Msg deleteCart(@PathVariable("goodsid")Integer goodsid, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if(user == null) {
            return com.toky.shop.pojo.Msg.fail("请先登录");
        }
    ...
    @ResponseBody
    public com.toky.shop.pojo.Msg updateCart(Integer goodsid, Integer num, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if(user == null) {
            return com.toky.shop.pojo.Msg.fail("请先登录");
        }
        ...
        return com.toky.shop.pojo.Msg.success("更新购物车成功");
    }
}
