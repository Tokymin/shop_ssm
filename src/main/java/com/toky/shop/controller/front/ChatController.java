...
//        chatExample.setOrderByClause("MsgTime asc");
    ...
    @ResponseBody
    public com.toky.shop.pojo.Msg getChatTo(HttpSession session, Model model, Integer sendto) {
        //查询历史消息聊天对象
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return com.toky.shop.pojo.Msg.fail("未登录");
        }
        ChatExample chatExample = new ChatExample();
        chatExample.or().andReceiveuserEqualTo(user.getUserid());
//        chatExample.or().andSenduserEqualTo(user.getUserid());
...
//        chatExample.setOrderByClause("MsgTime asc");
        ...
        return com.toky.shop.pojo.Msg.success("获取聊天列表成功").add("userlist",userList);
    }


    @RequestMapping("/getMessage")
    @ResponseBody
    public com.toky.shop.pojo.Msg getMessageInfo(Integer senduser, Integer receiveuser, HttpSession session) {
        ...
        return com.toky.shop.pojo.Msg.success("获取消息成功").add("message", chatList);
    }
    ...
    @ResponseBody
    public com.toky.shop.pojo.Msg adminChat(HttpSession session, Model model, Integer sendto) {
...
//        chatExample.setOrderByClause("MsgTime asc");
        ...
        List<User> userList = userService.selectByExample(userExample);
//        model.addAttribute("chatuserlist", userList);
        return com.toky.shop.pojo.Msg.success("获取列表成功").add("userlist",userList);
    }

    @RequestMapping("/sendMessage")
    @ResponseBody
    public com.toky.shop.pojo.Msg saveMessage(Chat chat) {
//        System.out.println(chat.getMsgcontent());
        chat.setMsgtime(new Date());
        chatService.insertChatSelective(chat);
        return com.toky.shop.pojo.Msg.success("保存成功!");
    }
